---
layout: page
title: Projetos
permalink: /projects/
---

# Projetos

Portfólio de projetos em **Pricing, Analytics e Inteligência de Mercado**, com foco em aplicação prática, tomada de decisão baseada em dados e impacto em resultados de negócio.

---

## 📌 Projetos em Destaque

---
### 📊 Monitor de Competitividade e Price Gap  
**Inteligência de Mercado · Concorrência · Price Index**

Sistema de monitoramento contínuo de preços da concorrência para identificação de gaps, dispersões e riscos competitivos.

- Repo: [Ver projeto no site]([https://ivancaputonunes-hub.github.io/projects/](https://ivancaputonunes-hub.github.io/monitor-competitividade-price-gap./))
- Stack: Python (Pandas), testes (pytest) e CI (GitHub Actions)
- Output: CSV e Parquet prontos para BI (Power BI/Looker/Metabase)

---

### 🧠 (WIP) Motor de Precificação Dinâmica para Varejo  
**Pricing Analytics · Simulação de Cenários · Margem**

Modelo analítico para recomendação de preços por SKU ou cluster, considerando custo, concorrência, elasticidade e regras de negócio.


---

### 📈 (WIP) Simulador de Estratégia Comercial  
**Preço × Volume × Margem · Estratégia Comercial**

Ferramenta de simulação para avaliação de trade-offs econômicos entre preço, volume, receita e margem antes da tomada de decisão.


---

## Como esses projetos se conectam

Os projetos foram pensados como **blocos complementares**:

- O **Monitor de Competitividade** garante alinhamento ao mercado  
- O **Motor de Precificação** recomenda preços ótimos  
- O **Simulador Comercial** avalia impactos estratégicos antes da execução  

Juntos, formam uma **base analítica completa para decisões comerciais orientadas por dados**.

---

⬅️ [Voltar para meu perfil no GitHub](https://github.com/ivancaputonunes-hub)






